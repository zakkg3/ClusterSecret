...
async def namespace_watcher(logger: logging.Logger, reason: kopf.Reason, meta: kopf.Meta, **_):
    ...
    for cached_cluster_secret in csecs_cache.all_cluster_secret():
        ...
        if reason == "delete" and ns_name in ns_list_synced:
            ...
            # Ensure that deleted namespace will not come in new list - at the moment this event is handled by kopf the namespace in kubernetes can still exist
...
async def on_secret_update(logger: logging.Logger, name: str, namespace: str, **_):
    """Watch for secret update events and re-sync ClusterSecrets that reference the updated secret via valueFrom
    """
    for cached_cluster_secret in csecs_cache.all_cluster_secret():
        body = cached_cluster_secret.body
        ...
        if ref_name is None or ref_namespace is None:
            continue

        if name == ref_name and namespace == ref_namespace:
            logger.info(f'Secret {name} in namespace {namespace}, which is linked with ClusterSecret {cached_cluster_secret.name} was changed. Re-syncing')
