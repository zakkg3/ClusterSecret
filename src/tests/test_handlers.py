...
from handlers import create_fn, custom_objects_api, csecs_cache, namespace_watcher, on_field_data, on_secret_update, startup_fn
...
class TestClusterSecretHandler(unittest.TestCase):
    ...
    def test_ns_delete(self):
        """Deleted namespace must be removed from cluster secret 'status.create_fn.syncedns' field.
        """
        ...
        # The syncedns status of the clustersecret should not contain the deleted namespace.
        patch_clustersecret_status.assert_called_once_with(
            logger=self.logger,
            name=csec.name,
