...
def sync_secret(
    ...
    try:
        ...
        if e.status == 404:
            logger.debug(f'Namespace {namespace} not found while syncing secret {sec_name}. Namespace was deleted during sync; this will be handled by the namespace deletion event handler.')
            return
        raise
